...
if(NOT HPCC_BUILD_TEST)
    set(HPCC_BUILD_TEST ON)
endif()

if(NOT HPCC_BUILD_BENCHMARK)
    set(HPCC_BUILD_BENCHMARK OFF)
endif()
...
if(HPCC_TARGET_OS)
    set(CMAKE_TARGET_OS ${HPCC_TARGET_OS})
endif()

if(HPCC_TARGET_ARCH)
    set(CMAKE_TARGET_ARCH ${HPCC_TARGET_ARCH})
endif()
...
macro(hpcc_set_architecture)
    ...
    unset(__ARCHITECTURES)
endmacro()
hpcc_set_architecture()
...
if(MSVC)
    ...
    if(CMAKE_C_COMPILER_ID STREQUAL "GNU")
        set(gnuc_options "-ffunction-sections -fdata-sections")
    ...
    if(IS_ARM)
        append_c_compiler_flag("-ftree-vectorize -flax-vector-conversions")
        append_cxx_compiler_flag("-ftree-vectorize -flax-vector-conversions")
    endif(IS_ARM)
    if(IS_ARMV7)
        foreach(lang C CXX ASM)
            string(FIND "${CMAKE_${lang}_FLAGS}" "-mfpu" OUTPUT)
            if(OUTPUT EQUAL "-1")
                append_compiler_flag(${lang} "-mfpu=neon-fp16")
            endif()
            string(FIND "${CMAKE_${lang}_FLAGS}" "-mfpu" OUTPUT)
            if(OUTPUT EQUAL "-1")
                append_compiler_flag(${lang} "-mfpu=neon")
            endif()
            string(FIND "${CMAKE_${lang}_FLAGS}" "-mfloat" OUTPUT)
            if(OUTPUT EQUAL "-1")
                if("${CMAKE_${lang}_COMPILER}" MATCHES ".+hf-")
                    append_compiler_flag(${lang} "-mfloat-abi=hard")
                else()
                    append_compiler_flag(${lang} "-mfloat-abi=softfp")
                endif()
            endif()
            string(FIND "${CMAKE_${lang}_FLAGS}" "-mfloat-format" OUTPUT)
            if(OUTPUT EQUAL "-1")
                append_compiler_flag(${lang} "-mfloat-format=ieee")
            endif()
        endforeach()
        append_compiler_flag(ASM "-mfpu=neon-fp16")
    endif(IS_ARMV7)
endif()
if(ANDROID)
  foreach(lang C CXX ASM)
...
if(HPCC_USE_CUDA OR WITH_CUDA OR USE_CUDA)
